...
def _present_answer(text: str) -> str:
    ...
    value = re.sub(
        r"No execution capability[^.]*\.",
        "This read-only reasoning component does not execute changes; remediation is available through the governed native approval path.",
        value,
        flags=re.IGNORECASE,
    )
    value = re.sub(
        r"Compliance Agent v1 can only report status[^.]*\.",
