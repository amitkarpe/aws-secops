/* Add the unified compliance MCP while preserving existing S3 reader/executor.
 * Run on the retained LibreChat host, then restart only the verified LibreChat service.
 */
...
const entry = {
  ...
  args: ['-m', 'pilot_v1.compliance_mcp'],
...
if (updated.mcpServers.aws_compliance && !isDeepStrictEqual(updated.mcpServers.aws_compliance, entry))
  throw Error('existing aws_compliance MCP differs; review before replacing');
updated.mcpServers.aws_compliance = entry;

const reads = ['get_config_summary','list_config_findings','list_sg_batches','get_sg_batch'];
for (const tool of reads) {
  const name = tool + '_mcp_aws_compliance';
  if (!approval.allow.includes(name)) approval.allow.push(name);
  if (approval.ask.includes(name)) throw Error('read tool unexpectedly configured as ASK');
}
...
if (rendered !== raw) {
  const backup = file + '.before-aws-compliance';
  if (!fs.existsSync(backup)) fs.writeFileSync(backup, raw, {mode:0o600,flag:'wx'});
  fs.writeFileSync(file + '.compliance-new', rendered, {mode:fs.statSync(file).mode & 0o777, flag:'wx'});
  fs.renameSync(file + '.compliance-new', file);
}
console.log('AWS_COMPLIANCE_MCP=READY READS=ALLOW SG_EXECUTOR=ASK S3_CONFIG=PRESERVED');
