...
UPDATE_FILES = [
    ...
    "integration/patch-librechat-approval-description.cjs",
    ...
    "tests/e2e/librechat_compliance_agent.py",
]
FULL_EXTRA_FILES = [
...
commands += [
    f"echo '{hashlib.sha256(payload).hexdigest()}  /opt/aws-secops/.runtime/sg-compliance.tgz' | sha256sum -c - >/dev/null",
    "tar -xzf /opt/aws-secops/.runtime/sg-compliance.tgz -C /opt/aws-secops-sg",
    "test -x /opt/aws-secops-sg/.venv/bin/python || python3 -m venv /opt/aws-secops-sg/.venv",
    "/opt/aws-secops-sg/.venv/bin/pip install --disable-pip-version-check -r /opt/aws-secops-sg/requirements-bulk.txt",
    "chmod -R a+rX /opt/aws-secops-sg/pilot_v1 /opt/aws-secops-sg/integration /opt/aws-secops-sg/scripts /opt/aws-secops-sg/.venv",
]
if not a.update_code_only:
    commands += stage("/var/lib/aws-secops-sg/manifest.json", a.manifest.read_bytes())
    commands += stage("/var/lib/aws-secops-sg/gateway.json", a.gateway_state.read_bytes())
    commands += ["chown ssm-user:ssm-user /var/lib/aws-secops-sg/*.json"]
...
commands += [
    ...
    "install -m 644 /opt/aws-secops-sg/integration/patch-librechat-approval-description.cjs /opt/aws-secops/integration/patch-librechat-approval-description.cjs",
    "node /opt/aws-secops/integration/patch-librechat-approval-description.cjs /opt/LibreChat",
    "install -m 644 /opt/aws-secops-sg/integration/update-compliance-v1-agent.cjs /opt/aws-secops/integration/update-compliance-v1-agent.cjs",
    "install -m 644 /opt/aws-secops-sg/integration/compliance-agent-v1.json /opt/aws-secops/integration/compliance-agent-v1.json",
    "cd /opt/LibreChat && node /opt/aws-secops/integration/update-compliance-v1-agent.cjs /opt/aws-secops/integration/compliance-agent-v1.json",
