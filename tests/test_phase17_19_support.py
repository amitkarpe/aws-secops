...
class OperatorPageTests(unittest.TestCase):
    def test_simple_two_card_ui_and_no_reset_all(self):
        ...
        self.assertIn("https://config.astromedicomp.org/", html)
        self.assertIn(">Compliance Agent</a>", html)
        self.assertIn(">Config Dashboard</a>", html)
        self.assertNotIn(">Open AWS Compliance Agent</a>", html)
        self.assertNotIn(">Open Config Dashboard</a>", html)
        ...
        self.assertIn("Detect → Plan → Human Approval → Bounded Fix → Verify", html)
        self.assertIn("Audit / evidence timeline", html)
        self.assertIn("8 live control checks", html)
        self.assertIn("COMPLIANT x4", (ROOT / "pilot_v1/operator_server.py").read_text())
        self.assertIn("acceptance", (ROOT / "pilot_v1/operator_server.py").read_text())
    ...
    def test_agent_has_unified_planner_but_no_reset_tool(self):
        ...
        self.assertIn("ACCEPTED MULTI-ACCOUNT PROOF", agent["instructions"])
        self.assertIn("lab-dev, lab-poc, lab-qa and lab-sec", agent["instructions"])
        self.assertIn("not proof that this chat runtime has generic multi-account write authority", agent["instructions"])
        self.assertIn("DEFAULT LIVE SCOPE", agent["instructions"])
        self.assertIn("get_multi_account_status_mcp_aws_compliance", agent["tools"])
        self.assertIn("get_multi_account_remediation_plan_mcp_aws_compliance_planner", agent["tools"])
